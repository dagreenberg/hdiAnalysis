---
title: "Simpson's simulation vignette for normal distribution"
author: "Andrew Edwards"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Simpson's simulation vignette for normal distribution}
...
<!-- To build either run
rmarkdown::render("simpsons-simulation-normal.Rmd")
...
Copying `simpsons-simulation.Rmd` but changing data to normally distributed
rather than lognormal, to test on symmetric distribution.
...
Simulated values are saved in the `sim` list object, so just use those. Use the
same named values as for lognormal simulation, though they're not the log.
...
maybe need to do for multiple different parameters. Then can just replace
`lnorm` with `norm`.
...
ints_simulations <- data.frame(sim_num = 0,   # 0 for theoretical, then easy to plot
                          full = pnorm(Inf, mean_log, sd_log),
                          int_to_mean = pnorm(mean_log, mean_log, sd_log),
                          int_to_100 = pnorm(100, mean_log, sd_log),   # 100 is a bit arbitrary
                          # Doing normalised kernels below, the theoretical
                          # values are the same here though:
                          full_norm = pnorm(Inf, mean_log, sd_log),
                          int_to_mean_norm = pnorm(mean_log, mean_log, sd_log),
                          int_to_100_norm = pnorm(100, mean_log, sd_log),
                          full_spatstat = 1,
                          int_to_mean_spatstat = pnorm(mean_log, mean_log, sd_log),
                          int_to_100_spatstat = pnorm(100, mean_log, sd_log))      # For simulations will calc
                                        # using spatstat
