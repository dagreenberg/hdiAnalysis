...
THIS is after fixing the zero issue with using min of data. BUT not rerunning
everything above, just want to check results didn't change for non-zero years
```{r checknonzero}
res_all_years_new <- res_all_years <- create_intervals(dplyr::select(hake_recruitment_mcmc,
                                                                     -"Virgin"))
years_zeros <- which()
HERE
TEST both, do again. Not sure I put zero option into create_intervals.data.frame()
```
