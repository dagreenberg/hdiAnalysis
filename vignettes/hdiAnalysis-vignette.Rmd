...
# Check remaining ones are the same
...
HERE need to include the integral calculations, maybe just in
`create_intervals()` since want to redo interval if integral is < 0.95.
Have done - think want to recalculate density early on if integral of it is not 1.
Need to decided if recalculate the density, or just scale the integral by the
total. First is properly less confusing, but may introduce errors. Maybe just do
the latter. Think about.
HERE. Deal with warnings, which are mine in `create_intervals(). Just do 1966
...
# Surpsising still doesn't cahnge the integral, though the plot looks smooth.

# Prob don't need all this. Tidy up.
res_1966 <- create_intervals(select(hake_recruitment_mcmc, "1966") %>% pull())

spatstat.geom::integral(res_1966$density,
                        domain = c(as.numeric(res_1966$intervals$eti_lower),
                                   as.numeric(res_1966$intervals$eti_upper)))


spatstat.geom::integral(res_1966$density,
                        domain = c(as.numeric(res_1966$intervals$hdi_lower),
                                   as.numeric(res_1966$intervals$hdi_upper)))
...
spatstat.geom::integral(res_2024$density,
                        domain = c(as.numeric(res_2024$intervals$eti_lower),
                                   as.numeric(res_2024$intervals$eti_upper)))


spatstat.geom::integral(res_2024$density,
                        domain = c(as.numeric(res_2024$intervals$hdi_lower),
                                   as.numeric(res_2024$intervals$hdi_upper)))

# But maybe the full integral does not equal 1. Definitely best to calculate all
# these in one go.
spatstat.geom::integral(res_2024$density,
                        domain = c(0, Inf))
# So the HDI does give 95% of the assumed distribution:
0.8705/0.9156
#  = 0.9507427
# HERE could tweak it down then to make it 95% of the distribution, assuming
# (conservatively) that values cannot be lower than the minimum value.
```


DO ME FIRST HERE

- First save the `allow_hdi_zero` as an output in create_intervals.
...
- also save integral over (0,Inf) and also over ETI and HDI.
- if integral is not 1 then could rescale it; in example above that made the HDI
  contain 95%.
...
```{r plotttt}
plot(res_2024, xlim= c (0, 40))

plot(res_2024, xlim= c (0, 1))
