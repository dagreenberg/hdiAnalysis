...
```
--
Copying `hdiAnalysis-vignette.Rmd` to redo here with `density = FALSE`, to
get numbers and plots for manuscript.
...
## Set up and look at a single vector of data
...
Our 'data' are the hake Markov chain Monte Carlo (MCMC) recruitment and relative biomass
values are saved as data objects in the package, with `rec_2021` being the
vector of 8,000 MCMC values used for our Fig. 1. To run this code on your own
data you just need to have your values in a vector, and replace `rec_2021` with
...
So to analyse a single vector of values users can just apply the following code
to create an equivalent to our Figure 1 and obtain equivalent numerical
values. We call the results here `res_vec` for results on a vector, so that the
subsequent code will not need changing by users.
...
```{r tab, results = "asis"}
summary_table(res_2021)
```

## MCMC samples for multiple years -- Figure 2A
...
(with the first column representing the Virgin unfished equilibrium biomass) and each of the 8,000
...
manually checked, code not output here).
...
population that are mature (roughtly half of age-2 females are considered mature).

Relative spawning biomass calculations (adapted from `report/hdi-analysis.Rmd`,
as hadn't actually included in any vignettes yet).
...
res_relative_biomass <- create_intervals(current_relative_biomass_mcmc_sort,
                                     n = 1e5)
res_relative_biomass$intervals %>% a()
plot(res_relative_biomass,
...
TODO need to redo this based on recruitment for all years (abvoe), this is
adapted from much older code
```{r allyears}
hake_relative_with_forecast <- cbind(hake_relative_biomass_mcmc,
                                       hake_relative_biomass_mcmc_forecast)
relative_biomass_res <- create_intervals(hake_relative_with_forecast)  # n=1e5 is hardwired
relative_biomass_res$intervals_all   # Also returns densities, so could
                                     # plot each year if wanted
relative_biomass_res$intervals_all %>% tail()
# Check with Table b, these all match (obviously no 2025+):
relative_biomass_res$intervals_all %>%
  select(quantity, eti_lower, median, eti_upper) %>%
  filter(quantity >= 2015) %>%
  mutate(`2.5` = eti_lower * 100,
         median = median * 100,
         `97.5` = eti_upper * 100) %>%
  a() %>%
...
# this is for plotting all distributions, may add back in. Maybe a different
#     vignette. TODO
#     type = "hdi")
#     xlim = c(0, 40),
#     xlab = "Recruitment (billions of fish)",
...
```{r whichyears}
filter(relative_biomass_res$intervals_all,
       eti_lower > 0.4,
       hdi_lower < 0.4)
```
...
```{r sbdensityfalse}
relative_biomass_all_years <- create_intervals(hake_relative_with_forecast,
                                               density = FALSE)
relative_biomass_all_years$intervals_all %>% tail()

filter(relative_biomass_all_years$intervals_all,
       eti_lower > 0.4,
       hdi_lower < 0.4)

plot(relative_biomass_all_years,
     y_max = 2.6,
     xlim = c(2010, 2027),
     add_line_at_0.4 = TRUE)
...
TODO decided what to do with this, prob keep but just short
...
TODO Keep for table example (for me to then use R Markdown), but then can delete).
```{r tableres}
knitr::kable(res_2021$intervals %>% select(median,
                                           eti_lower,
                                           eti_upper,
                                           hdi_lower,
                                           hdi_upper,
