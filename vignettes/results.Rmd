...
adapted for users to apply to their own data or model outputs.

TODO Do lots here, then make a short (just reproducing results and numbers, and
as a template for others) and long version maybe. And change names to `my_data`
or something so people can easily adapt for their own data.
...
We present what works, to be used as a template for other analyses, and TODO
[might not need] then
change some code settings to show what did not work for our investigations.



## Set up and look at data
...
The 'data' are the hake Markov chain Monte Carlo (MCMC) recruitment and relative biomass
...
to create an equivalent to our Figure 1 and obtain equivalent numerical values.

TODO go through and standardise things to make them easily adaptable.
