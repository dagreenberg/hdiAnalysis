...
Check the integral with our `integrate_simpsons()` function to evaluate an integral on
a density object (our function gives very similar results to the
`spatstat.geom::integral()` one that we originally used, but that messed up the testing):
```{r rec2}
...
than the true 95% ETI interval. This is because it has not used the density, which could
...
HERE  - may need to tweak that.
what's below with this. First reproduce existing plot, then redo for density = FALSE. But doing density = FALSE below and used that, doing that just to llok at numbers before going home.
#     type = "hdi")
#     xlim = c(0, 40),
#     xlab = "Recruitment (billions of fish)",
#     mfrow = c(60, 1))  # 59 default

# OLD, as used in first main draft, used limits etc. as template, before standardised functions. Can't find where I used new plot function on biomass
plot_series(relative_biomass_res$intervals_all,
            y_max = 2.6,
            xlim = c(2010, 2027),
            add_line_at_0.4 = TRUE,
            inc = 0.05,
