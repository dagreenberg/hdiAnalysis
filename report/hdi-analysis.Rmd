...
Do here then make a function (TODO), generalised for recruitment also
```{r allyears}
res_all_years <- list()
intervals_all_years <- tibble()
for(i in 1:ncol(hake_relative_biomass_mcmc)){
  this_year = as.numeric(names(hake_relative_biomass_mcmc))[i]

  this_year_mcmc <- pull(hake_relative_biomass_mcmc,
...
intervals_all_years %>% a()
```

Test my new function:
```{r testfun}
new_res <- calc_density_many_years(hake_relative_biomass_mcmc)
expect_equal(intervals_all_years,
             new_res$intervals_all_years)
new_res$intervals_all_years
```

Now a quick plot:
```{r biomassseries}
plot(intervals_all_years$year,
     intervals_all_years$median,
     type = "o",
     ylim = c(0, 2.5),
     xlab = "Year",
     ylab = "Relative spawning biomass")
lines(intervals_all_years$year,
      intervals_all_years$`2.5`,
      col = "blue")
lines(intervals_all_years$year,
      intervals_all_years$hdi_lower,
      col = "red")

lines(intervals_all_years$year,
      intervals_all_years$`97.5`,
      col = "blue")
lines(intervals_all_years$year,
      intervals_all_years$hdi_upper,
      col = "red")
