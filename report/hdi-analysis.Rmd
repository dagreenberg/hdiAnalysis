...
TO DO: add arrows with equal-atil to explain 2.5%, add 95%, and line to hdi
...
```{r allyears}
relative_biomass_res <- calc_density_many_years(hake_relative_biomass_mcmc)  # n=1e5 is hardwired
relative_biomass_res$intervals_all_years   # Also returns densities, so could
                                        # plot each year if wanted
```
Now a quick plot: TODO make a function
```{r biomassseries}
plot(relative_biomass_res$intervals_all_years$year,
     relative_biomass_res$intervals_all_years$median,
     type = "o",
     ylim = c(0, 2.5),
     xlab = "Year",
     ylab = "Relative spawning biomass")
lines(relative_biomass_res$intervals_all_years$year,
      relative_biomass_res$intervals_all_years$`2.5`,
      col = "blue")
lines(relative_biomass_res$intervals_all_years$year,
      relative_biomass_res$intervals_all_years$hdi_lower,
      col = "red")

lines(relative_biomass_res$intervals_all_years$year,
      relative_biomass_res$intervals_all_years$`97.5`,
      col = "blue")
lines(relative_biomass_res$intervals_all_years$year,
      relative_biomass_res$intervals_all_years$hdi_upper,
      col = "red")
...
So, not too important for historical trajectory. Will focus on, say, last 10
years, and do two years of projections (TODO Andy get the full MCMC projection
results).
...
```
Very quick plot, can adapt the usual recruitment style:
```{r recquickplot}
plot(recruitment_intervals$year,
     recruitment_intervals$median,
     type = "o",
     ylim = c(0, 30),
     xlab = "Year",
     ylab = "Recruitment")
lines(recruitment_intervals$year,
      recruitment_intervals$`2.5`,
      col = "blue")
lines(recruitment_intervals$year,
      recruitment_intervals$hdi_lower,
      col = "red")

lines(recruitment_intervals$year,
      recruitment_intervals$`97.5`,
      col = "blue")
lines(recruitment_intervals$year,
      recruitment_intervals$hdi_upper,
      col = "red")
```

There were some warnings, and some of the `hdi_lower` come out negative. TODO
look into those. Hmmm......
