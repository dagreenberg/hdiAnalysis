...
changing `density()` option `from`, it should just get passed through the functions:
...
### Just try one year to get it working - can delete once done above.

This code works for year being 2021, but not 1966 or 1969 (which has HDI to
0). Works for 1970 (HDI not to 0). So need to dig into `plot_density()`.

```{r onefrom}
one_year_mcmc_1966 <- pull(hake_recruitment_mcmc,
                           `1966`)    # Full 8000 MCMC samples for that year

one_year_res_1966 <- calc_density(one_year_mcmc_1966,
                                  from = 0,
                                  n = 1e5)
one_year_res_1966
one_year_res_1966$intervals %>% a()
plot_density(dat_mcmc = one_year_mcmc_1966,
             dens_intervals = one_year_res_1966)
```

Aha, the issue is the first y value (at x=0) is not 0 (is actually quite high),
and so that messes up the polygons:
```{r onefromvals}
head(one_year_res_1966$density$x)
head(one_year_res_1966$density$y)
```
and just plotting the density shows this (and it's not just minor):
```{r onefromdens}
plot(one_year_res_1966$density, main = "Recruitment in 1966")
```
