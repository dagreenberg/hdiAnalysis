...
calc_density <- function(dat_mcmc,
  ...
  y_low_equal_interp <- dens$y[i_low_equal] + (dens$y[i_low_equal + 1] -
                                        dens$y[i_low_equal])/(dens$x[i_low_equal + 1] - dens$x[i_low_equal]) *
  ...
  y_high_equal_interp <- dens$y[i_high_equal] + (dens$y[i_high_equal + 1] -
                                          dens$y[i_high_equal])/(dens$x[i_high_equal + 1] - dens$x[i_high_equal]) *
    (intervals$`97.5` - dens$x[i_high_equal])

  # HDI
  i_low_hdi <- findInterval(intervals$hdi_lower,
                            dens$x)  # low is between dens$x[i_low] and dens$x[i_low + 1]
  y_low_hdi_interp <- dens$y[i_low_hdi] + (dens$y[i_low_hdi + 1] -
                                    dens$y[i_low_hdi])/(dens$x[i_low_hdi + 1] - dens$x[i_low_hdi]) *
  ...
  y_high_hdi_interp <- dens$y[i_high_hdi] + (dens$y[i_high_hdi + 1] -
                                      dens$y[i_high_hdi])/(dens$x[i_high_hdi + 1] - dens$x[i_high_hdi]) *
